...
for Fnum, cases in simulations.items():
    ...
    for sec, subsec in cases:
        ...
        for idx, file_path in enumerate(file_list):
            ...
            print(f"   [{idx + 1}/{len(file_list)}] Rendering Bottom Temperature for: {base_filename}")

            with xr.open_dataset(file_path) as ds:
                # -----------------------------------------------------------------
                # Extract Salinity (Required for the 34.9 contour line)
                # -----------------------------------------------------------------
                ...
                # -----------------------------------------------------------------
                # Extract Temperature (Required for background fill color)
                # -----------------------------------------------------------------
                if 'timeMonthly_avg_activeTracers_temperature' in ds:
                    ...
                    print(f"   --> Error: Temperature key configuration mismatch in {base_filename}. Skipping file.")
                    continue
            ...
            # Map Limits and Colormap Specifications for Temperature
            v_min, v_max = -2.6, -1.8  # Adjust these bounds if needed for your temperature range
            norm = colors.Normalize(vmin=v_min, vmax=v_max)
            varcmap = "cmo.thermal"
            plotlabel = "Sea Floor Temperature [°C]"
            ...
            # Native MPAS unstructured mesh interpolation via Mosaic (plotting Temperature)
            descriptor = mosaic.Descriptor(dsMesh_trimmed, projection=projection)
            collection = mosaic.polypcolor(ax, descriptor, bottom_temp, norm=norm, cmap=varcmap)

            # Overlay Specific Isohaline Contours (34.9 psu critical threshold from Salinity)
            ...
            ax.set_title(f"Bottom Temperature & 34.9 Salinity Contour - {dx} {sec} {subsec_str} ({date_str})", fontsize=12, pad=10)
