...
# =========================================================================
# Choose what to plot: 'area' or 'volume'
PLOT_VARIABLE = 'volume'  # Options: 'area' or 'volume'

if PLOT_VARIABLE == 'area':
...
def process_single_file(file_path, idx, total_files, dx, sec, subsec_str, opt_region, iis, lon, lat,
    ...
    """Processes a single mpassi history file and generates a sea ice plot."""
    ...
    with xr.open_dataset(file_path) as ds:
        # Dynamically check for chosen variable, fall back to non-timeMonthly name if needed
        if NC_VAR_NAME in ds:
            ice_data = ds[NC_VAR_NAME].isel(Time=0).values
        elif PLOT_VARIABLE == 'area' and 'iceAreaCell' in ds:
            ice_data = ds['iceAreaCell'].isel(Time=0).values
        elif PLOT_VARIABLE == 'volume' and 'iceVolumeCell' in ds:
            ice_data = ds['iceVolumeCell'].isel(Time=0).values
        else:
            return f"Error: Required sea ice variable ({NC_VAR_NAME}) missing in {base_filename}"

    # Apply region restrictions if toggled
    if opt_region:
        ice_data = np.where(iis == 0, np.nan, ice_data)
    ...
    # Dynamic norm bounds configured from top-level choice
    collection = mosaic.polypcolor(ax, descriptor, ice_data, norm=colors.Normalize(vmin=VMIN, vmax=VMAX),
                                   cmap="cmo.ice")
    ...
    fig.colorbar(collection, fraction=0.1, shrink=0.5, label=CBAR_LABEL)
    ax.set_title(f"{TITLE_PREFIX} - {dx} {sec} {subsec_str} ({date_str})", fontsize=12, pad=10)

    # Write output to sea ice snapshots directory with dynamic prefix
    output_png_path = f'{out_plot_dir}/{FILE_PREFIX}_{dx}_{sec}{subsec_str}_{date_str}.png'
