...
melt_total = np.zeros((len(vlay),len(sgr)))
h = 0
for v in range(len(vlay)):
    for s in range(len(sgr)):
        if v == 0:
            fdir = f'{p_base}/{temp[t]}/{temp[t]}_{hloc[h]}{sgr[s]}{suf[v]}'
        else:
            fdir = f'{p_base}/rdvx/{temp[t]}/{temp[t]}_{hloc[h]}{sgr[s]}{suf[v]}'
...
smb = 'oo^^'
for v in range(len(vlay)):
    plt.plot(sgr_val, melt_total[v,:], f'{clr[v]}:', linewidth=1, marker=f'{smb[v]}', fillstyle='full', markersize=4, label = f'{vlay[v]}')
...
if opt_save == 1:
    plt.savefig(f'{dir_fig_save}/plot_bulk_compare_v_res.png', bbox_inches='tight', dpi=300)
else:
    plt.show()

plt.figure(figsize=(4, 4))
for v in range(len(vlay)):
    plt.loglog(sgr_val, melt_total[v,:], f'{clr[v]}:', linewidth=1, marker=f'{smb[v]}', fillstyle='full', markersize=4, label = f'{vlay[v]}')
    popt, pcov = curve_fit(f_n_pow_2_3, sgr_val, melt_total[v, :])
...
if opt_save == 1:
    plt.savefig(f'{dir_fig_save}/plot_bulk_compare_v_res_lglg.png', bbox_inches='tight', dpi=300)
else:
    plt.show()
